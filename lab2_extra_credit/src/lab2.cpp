...
#define MAXIMUM_CORRECT_ARGS_NUM 4
#define CONFIG_FILE_OFFSET 1
#define MIN_PLAYER_NUM_OFFSET 2
#define OVERRIDE_FLAG_OFFSET 3
#define PATH
...
main(int argc, char* argv[])
{
    ...
    if ( argc > MINIMUM_CORRECT_ARGS_NUM)
    {
        ...
        catch(...)
        {
            ...
            cerr <<  "USAGE: " << argv[0] << "<script_file_name> [<minimum number of players to use>, -override]" << endl;
            return EINVAL;
        }
        if (arg <= 0)
        {
    ...
    if ( argc > OVERRIDE_FLAG_OFFSET)
    {
        override_flag = (strcmp(argv[OVERRIDE_FLAG_OFFSET], "-override") == 0);
        if ( !override_flag )
        {
            cerr << "Error: unacceptable third argument." << endl;
            cout << "USAGE: " << argv[0] << "<script_file_name> [<minimum number of players to use>, -override]" << endl;
            return EINVAL;
        }
    ...
    Director* director = new Director(argv[CONFIG_FILE_OFFSET], min_plyer_num, override_flag);
    delete director;
