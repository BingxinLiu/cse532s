...
public:
    server_read() : has_not_released(true), id(client_counter), server_interceptor(nullptr)
    {
    ...
    virtual int handle_input (ACE_HANDLE h = ACE_INVALID_HANDLE)
    {
        ...
        if ( recv_len != 2 && this->server_interceptor != nullptr )
        {
            for (size_t i = 0; i < recv_len -1; ++i)
                this->server_interceptor->operator()(buffer[i], 2);
            buffer[recv_len] = '\0';
        }
