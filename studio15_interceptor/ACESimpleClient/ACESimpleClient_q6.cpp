...
class client : public ACE_Event_Handler
{
    ...
    interceptor_composer* composer;

public:

    client(int argc, char* argv[]) : argc((size_t)argc), argv(argv), composer(nullptr) {}

    void set_composer(interceptor_composer* composer)
    {
    ...
    virtual int handle_timeout (const ACE_Time_Value& ace_time_value, const void* _ )
    {
        ...
        string str = "0";
        // if ( this->client_interceptor != nullptr )
        // {
        //     transform(str.begin(), str.end(), str.begin(),[this](char c) -> char {
        //         return this->client_interceptor->operator()(c, 12);
        //     });
        // }
        ...
        for (size_t i = 1; i < argc; ++i)
        {
            ...
            cout << "send length = " << send_str.length() << endl;
            if ( this->composer != nullptr )
            {
                for (size_t j = 0; j < send_str.length(); ++j)
                    this->composer->execute(send_str[j]);
            }
...
main(int argc, char* argv[])
{
    ...
    mod_interceptor mod;
    rotation_interceptor rotation;
    interceptor_composer composer;
    composer.compose(&mod, 2);
    composer.compose(&rotation, 2);
    client client(argc, argv);
    client.set_composer(&composer);
