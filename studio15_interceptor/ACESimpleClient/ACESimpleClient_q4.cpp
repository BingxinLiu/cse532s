...
public:
    ...
    client(int argc, char* argv[]) : argc((size_t)argc), argv(argv), client_interceptor(nullptr) {}

    void set_interceptor(interceptor* intercptr)
    {
        client_interceptor = intercptr;
    }
        ...
        for (size_t i = 1; i < argc; ++i)
        {
            ...
            if ( this->client_interceptor != nullptr )
            {
                for (size_t j = 0; j < send_str.length(); ++j)
                    this->client_interceptor->operator()(send_str[j], 2); 
            }
...
main(int argc, char* argv[])
{
    ...
    ror_interceptor intercptr;
    client client(argc, argv);
    client.set_interceptor(&intercptr);
